...
class MTKTest {
    ...
    fun `공백 테스트 `() {
        ...
        assertEquals("십 이만 삼천 사백 오십 육원", MTK.toKorean(amount))
        assertEquals("십 이만삼천 사백 오십 육원", MTK.toKorean(amount, useSpacing = false))
        assertEquals("십이만 삼천사백오십육원", MTK.toKorean(amount, innerSplit = false))
        assertEquals("십 이만 삼천 사백 오십 육 원", MTK.toKorean(amount,spaceBeforeWon = true))
    }
}